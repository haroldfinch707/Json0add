class JSONViewer {
    ...
    initEventListeners() {
        ...
        // File upload handlers with proper error checking
        ...
        if (clearReviewed) {
            ...
            console.log('✅ Clear reviewed event listener attached');
        }
        ...
        if (exportData) {
            ...
            console.log('✅ Export data event listener attached');
        }
    ...
    showReport(id) {
        ...
        const report = this.generateBugBountyReport(item);
    ...
    generateBugBountyReport(item) {
        return {
            title: `Exposed ${item.detector || 'Secret'} in ${this.extractRepoName(item.repo_url)}`,
            summary: `A ${item.detector || 'secret'} was discovered in the repository. This credential exposure could potentially allow unauthorized access to sensitive systems and data.`,
            poc: `**Repository:** ${item.repo_url || 'Unknown'}\n**File Path:** ${item.file_path || 'Unknown'}\n**Commit:** ${item.commit || 'Unknown'}\n**Secret Type:** ${item.detector || 'Unknown'}\n\n**Evidence:**\nThe exposed secret was found at the following location:\n\`\`\`\n${item.raw_secret || 'Hidden for security'}\n\`\`\``,
...
console.log('🚀 Starting JSON Viewer initialization...');
